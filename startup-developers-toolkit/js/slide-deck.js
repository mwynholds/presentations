...
SlideDeck.prototype.buildNextItem_ = function() {
  ...
  if (built && ! slide.querySelector('.build-first'))
    built.classList.add('build-first')
  ...
  if (built) {
    ...
    if (built.classList.contains('fadeout')) {
      built.classList.add('build-fadeout');
    }
  }

  var replaceAll = function(from, to) {
    var items = slide.querySelectorAll('.' + from);
    for (var j = 0, item; item = items[j]; j++) {
      item.classList.remove(from);
      if (to != null) item.classList.add(to);
    }
  }

  if (!toBuild) {
    replaceAll('build-fade', null);
    return false;
  }

  slide.addEventListener( 'slideleave', function(evt) {
    setTimeout( function() {
      replaceAll('build-fadeout', null);
      replaceAll('to-build-save', 'to-build');
      replaceAll('build-first', 'build-current');
    }, 500 );
  });

  toBuild.classList.remove('to-build');
  toBuild.classList.add('to-build-save');
